  ...
  for (const m of matches) {
    ...
    if (isGroupMatch(m)) {
      if (hasScore(m)) {
        ...
        stats[t1].matchResults.push({ label: `${t1} ${s1}–${s2} ${t2}`, result: res1, pts: pts1, round: 'Group stage', date: m.date });
        stats[t2].matchResults.push({ label: `${t1} ${s1}–${s2} ${t2}`, result: res2, pts: pts2, round: 'Group stage', date: m.date });
      } else {
      ...
      if (hasScore(m)) {
        ...
        stats[t1].matchResults.push({ label: scoreStr, result: t1 === winner ? 'win' : 'loss', pts: null, round: m.round, date: m.date });
        stats[t2].matchResults.push({ label: scoreStr, result: t2 === winner ? 'win' : 'loss', pts: null, round: m.round, date: m.date });
        if (m.round === 'Final') {
...
function calcParticipantScore(participant, teamStats) {
  ...
  for (const team of participant.teams) {
    ...
    if (!s) {
      breakdown.push({ ...team, groupPts: 0, knockoutPts: 0, bucketBonus: 0, total: 0, bonusItems: [], matchResults: [] });
      continue;
    }
    ...
    for (const round of KNOCKOUT_ROUNDS) {
      if (s.knockoutRoundsReached.has(round)) {
        ...
        bonusItems.push({ label: `Reached ${round}`, pts: KNOCKOUT_BONUSES[round] });
      }
    }
    if (s.wonWorldCup) {
    ...
    let bucketBonus = 0;
    if (BUCKET_KNOCKOUT_BONUS[team.bucket] && s.knockoutRoundsReached.size > 0) {
      bucketBonus = BUCKET_KNOCKOUT_BONUS[team.bucket];
      bonusItems.push({ label: `Bucket ${team.bucket} underdog bonus`, pts: bucketBonus });
    }

    const teamTotal = groupPts + knockoutPts + bucketBonus;
    total += teamTotal;
    breakdown.push({ ...team, groupPts, knockoutPts, bucketBonus, total: teamTotal, bonusItems, matchResults: s.matchResults });
  }
...
function renderBoard(teamStats) {
  ...
  scored.forEach(p => {
    ...
    // Team chips
    const teamsHTML = p.breakdown.map(t => {
      const ptsHTML = t.total > 0 ? `<span class="tpts">+${t.total}</span>` : '';
    ...
    const resultsHTML = p.breakdown.map(t => {
      const matchRows = (t.matchResults.length === 0)
        ...
        : t.matchResults.map(r => {
          ...
          if (r.result === 'loss') { ptsLabel = '+0 pts'; ptsClass = 'loss'; }
      ...
      const bonusRows = t.bonusItems.map(b =>
...
function renderGroupTables(matches) {
  ...
  for (const m of matches) {
    ...
    [t1, t2].forEach(t => {
      if (!groups[grp][t]) groups[grp][t] = { w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0, played: 0 };
    });
    ...
    if (hasScore(m)) {
      ...
      if (s1 > s2) {
        groups[grp][t1].w++; groups[grp][t1].pts += 3;
        groups[grp][t2].l++;
      } else if (s1 < s2) {
        groups[grp][t2].w++; groups[grp][t2].pts += 3;
        groups[grp][t1].l++;
      } else {
        groups[grp][t1].d++; groups[grp][t1].pts++;
        groups[grp][t2].d++; groups[grp][t2].pts++;
      }
    }
  }
